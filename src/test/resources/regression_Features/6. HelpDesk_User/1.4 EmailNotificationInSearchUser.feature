@regression
@hd
Feature: IC Helpdesk user can view and select/deselect the Email Notification in Search User tab

  Background:
    Given TestName "Email Notification settings in IC Helpdesk user" the landing page is loaded
  ...
  Scenario: Verify Email Notification check box options can be selected
    When clicks on Email Preference settings icon
